...
type Runtime struct {
	...
	// SendMail dispatches a MailMessage through the core mail pipeline. It
	// runs the mail.before_send hook, applies core validation (CRLF
	// stripping, header normalisation, envelope resolution) and then hands
	// off to the configured transport. mail.after_send observes the
	// outcome. A nil implementation means the host has not enabled mail
	// dispatch and callers should treat mail as best-effort.
	SendMail func(context.Context, MailMessage) error
	// AvailableLanguages returns the set of language tags currently
	// recognised by the host i18n table. Route negotiators use this to
	// clamp their choices to a supported subset.
	AvailableLanguages func(context.Context) []string
	// NegotiateLanguage runs the route.language_negotiate pipeline for the
	// given request and returns the language a plugin should use for
	// downstream translation. It is a convenience wrapper over
	// DispatchHook and never returns an empty string — falling back to the
	// site default when no plugin claims the request.
