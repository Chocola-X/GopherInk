...
| `OpenSQLiteDatabase(ctx, filename)` | 打开当前插件/主题独占 SQLite 数据库，返回 `*sql.DB` 和文件路径 |
| `OpenSQLiteFor(ctx, owner, filename)` | 显式打开指定扩展 owner 的 SQLite 数据库 |
| `SQLitePath(owner, filename)` / `SQLiteSize(owner, filename)` | 查询独占数据库路径和占用空间 |
| `ClearSQLite(ctx, owner, filename)` | 关闭并删除独占数据库及其 WAL/SHM 文件 |
...
大量写入型插件不要把访问日志、统计明细等高频数据写入核心表。插件可以声明并打开自己的 SQLite 数据库文件，文件位于当前 `data` 目录下的 `extensions/plugin-<插件名>/`，会跟随 `GOPHERINK_DATA_DIR` 或启动配置的数据目录变化。主题使用同一接口时位于 `extensions/theme-<主题名>/`。数据库连接由核心缓存复用，使用 SQLite WAL、`busy_timeout` 和单写连接设置，适合访客记录这类高频追加写入。

声明数据库后，后台插件管理列表会显示文件名、占用空间，并提供“清除数据库”按钮。清除会关闭缓存连接并删除 `.db`、`-wal` 和 `-shm` 文件，不会修改插件配置。

```go
func (Plugin) SQLiteDatabases() []plugin.SQLiteDatabase {
    return []plugin.SQLiteDatabase{{
        Name:        "visits",
        Filename:    "visits.db",
        Label:       "访问记录",
        Description: "保存 PV、IP、UA 和路由访问明细",
    }}
}

func (Plugin) Init(m *plugin.Manager) {
    m.RegisterRuntimeHook(plugin.HookRequestAfter, recordVisit)
}
...
func recordVisit(ctx context.Context, rt *plugin.Runtime, value any) (any, error) {
    ...
    db, _, err := rt.OpenSQLiteDatabase(ctx, "visits.db")
    if err != nil {
        return value, err
    }
    ...
    _, err = db.ExecContext(ctx, `INSERT INTO visits
        (created, method, path, ip, user_agent, status, duration_ms)
        VALUES (?, ?, ?, ?, ?, ?, ?)`,
        time.Now().Unix(), payload.Method, payload.Path, payload.IP,
        payload.UserAgent, payload.Status, payload.Duration)
    return value, err
}
```

数据库 `Name` 和文件名只允许字母、数字、点、下划线和短横线；文件名会自动补 `.db` 后缀。插件仍需自行维护自己的表结构和索引策略。高频记录建议在钩子里先过滤后台、静态资源、机器人 UA 或不需要统计的路径，避免无意义写入。
...
- 保存明细：使用独占 SQLite 数据库，避免核心数据库随访问日志膨胀。
...
9. 高频记录插件使用独占 SQLite 数据库，并为常用查询建立索引。
