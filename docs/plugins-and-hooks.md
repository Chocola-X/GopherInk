...
func (Plugin) Description() string { return "Example plugin." }
...
func (Plugin) Translate(lang, key string) string {
...
| `ListThemeFiles(ctx, themeName...)` | 列出可编辑的非嵌入主题文件；不传主题名时使用当前启用主题 |
| `ThemeEditableDir(ctx, themeName...)` | 读取可编辑主题目录；嵌入主题或不可编辑主题返回 `false` |
...
| `SaveComment(ctx, input)` / `DeleteComment(ctx, coid)` | 通过核心编排层创建、更新或删除评论，会触发评论保存/删除钩子 |
...
Runtime 写入接口使用 `plugin.ContentWriteInput` 和 `plugin.CommentWriteInput`，不会要求插件导入 `core/services`。核心会把它们转换为服务层输入，并统一执行 `content.before_save`/`content.after_save`、`comment.before_save`/`comment.after_save`、删除前后钩子和内容附件删除策略。插件必须显式传入内容 `AuthorID`；评论 `OwnerID` 留空时会按 `CID` 尝试补齐内容作者。
...
`ThumbnailURL` 返回的是 GopherInk 后台缩略图端点，主要用于后台插件页面、管理卡片和已授权的管理视图。当前核心不提供任意尺寸公开图片处理 URL，`width`、`height` 参数仅为未来兼容保留；公开主题页面应优先使用附件原始 URL，或由插件通过 `HookAttachmentURL` / 自定义路由实现 CDN/图片代理策略。
...
核心会在每次调用插件钩子时隔离 `panic`：异常会连同插件名称和调用栈写入服务日志，并转换为普通钩子错误来停止当前调用链，避免请求完成后的异步钩子使整个进程退出。该保护只负责维持进程可用性；插件仍应正常返回错误，并自行保证外部资源和已开启事务得到释放。
...
| `HookContentParse` | `content.parse` | `ContentParserPayload` | 接管 Markdown 或普通文本解析，`Mode` 区分模式 |
| `HookExcerpt` | `content.excerpt` | `ExcerptPayload` | 过滤摘要结果 |
| `HookExcerptAfterRender` | `excerpt.after_render` | `ExcerptAfterPayload` | 过滤已渲染摘要 HTML |
...
| `HookContentPermalink` | `content.permalink` | `ContentPermalinkPayload` | 修改内容公开链接 |
| `HookContentAuthor` | `content.author` | `ContentAuthorPayload` | 修改内容作者公开信息 |
...
| `HookCommentGuard` | `comment.guard_validate` | `CommentGuardPayload` | 过滤匿名评论守卫校验结果 |

`HookCommentGuard` 在核心完成默认评论守卫校验后触发。Payload 中的 `Valid` 是核心结果，插件可根据请求、IP、UA、额外表单字段或自有数据库进一步置为 `false`。如果需要替换内置策略并把核心失败结果改为通过，必须同时设置 `Valid=true` 和 `Handled=true`；这样可以避免普通增强插件意外放宽默认防护。返回错误会按校验失败处理。该钩子用于反垃圾策略扩展，不负责渲染取消回复按钮这类纯前端交互。
...
| `HookArchiveBeforeQuery` | `archive.before_query` | `ArchivePayload` | 归档查询前过滤或接管 |
| `HookArchiveAfterQuery` | `archive.after_query` | `ArchivePayload` | 归档查询后过滤 |
| `HookArchiveBeforeRender` | `archive.before_render` | `ArchivePayload` | 归档模板渲染前过滤数据 |
| `HookArchiveAfterRender` | `archive.after_render` | `ArchivePayload` | 归档模板渲染后通知 |
| `HookArchiveSearch` | `archive.search` | `ArchivePayload` | 搜索查询前过滤或接管 |
| `HookMetaPermalink` | `meta.permalink` | `MetaPermalinkPayload` | 修改分类/标签公开链接 |
...
- `Tone`：MDUI 色调，如 `primary`、`secondary`、`tertiary`。
...
`ContentWriteInput.Fields` 中使用的字段输入结构：
...
func (Plugin) DatabaseTables() []plugin.TableDefinition {
    return []plugin.TableDefinition{{
        ...
        Columns: []plugin.ColumnDefinition{
            ...
            {Name: "created", Type: plugin.ColDatetime},
            ...
            {Name: "score", Type: plugin.ColFloat},
            {Name: "active", Type: plugin.ColBool, Default: "1"},
        },
        Indexes: []plugin.IndexDefinition{
            {Name: "created", Columns: []string{"created"}},
            {Name: "path", Columns: []string{"path"}},
            {Name: "path_unique", Columns: []string{"path"}, Unique: true},
        },
    }}
}
