...
- 新增或修改主题、插件后需要重新编译；插件和主题使用统一构建器自动发现。
...
`make list-components` 或 `go run ./cmd/gopherink-builder -list` 会列出参与构建的插件和主题。构建器扫描 `plugins/` 与 `themes/` 的直接子目录：目录根部必须包含至少一个非测试 `.go` 文件；目录可属于主项目 module，也可带有独立 `go.mod`。独立 module 会加入仅本次编译使用的临时 workspace，根目录 `go.mod` 和 `go.sum` 不会被改写。插件目录可以为空或不存在；主题目录必须至少发现一个可构建主题，否则构建报错。直接运行 `go build ./cmd/gopherink` 不执行目录发现，因此不适合构建外置插件或主题。
...
7. 变更插件或主题后先用 `make list-components` 核对发现结果，再重新编译，并在上线前验证其路由、钩子或主题切换。
