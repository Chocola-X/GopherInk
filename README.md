...
插件和主题采用构建时集成：每次新增或修改插件、主题源码，都需要重新编译并重启 GoBlog；后台启停不会热加载代码。
...
首次启动时，如果未设置管理员环境变量且数据库为空，程序会启用 Web 安装向导（`GOBLOG_WEB_INSTALL=true`），可通过浏览器完成站点初始化和管理员账户创建。
...
| `GOBLOG_DB_DRIVER` | 数据库驱动（`sqlite3`/`mysql`/`mariadb`/`postgres`） | 交互选择 |
...
| `GOBLOG_ADMIN_MAIL` | 初始管理员邮箱 | `admin@example.com` |
| `GOBLOG_WEB_INSTALL` | 启用 Web 安装向导 | `true` |
...
设置 `GOBLOG_DB_READ_DSN` 和 `GOBLOG_DB_WRITE_DSN` 后，读操作走读库、写操作走写库。也可在后台"基本设置"中配置 `db_read_dsn` / `db_write_dsn`。读库为空时自动回退到写库。
...
GoBlog 内置 Web 应用防火墙（WAF），提供多层安全防护。已认证管理员的后台请求自动跳过 WAF 检查。
...
GoBlog 提供 80+ 项站点配置，均可在后台管理界面修改。以下为主要配置分类：

### 基本设置

| 选项 | 默认值 | 说明 |
|------|--------|------|
| `site_title` | GoBlog | 站点标题 |
| `site_description` | 一个极简的 Go 博客 | 站点描述 |
| `site_keywords` | — | 站点关键词 |
| `base_url` | http://localhost:8080 | 站点 URL |
...
| `http_client_user_agent` | GoBlog/1.0 | User-Agent |
